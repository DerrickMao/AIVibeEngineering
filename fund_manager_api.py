...
2. **提取原始观点**：将该基金经理在文章中表达的核心观点原文提取出来。
3. **板块打标**：根据观点内容，从以下板块中选取涉及的板块（可多选，无则为空数组）：
   {sectors}
4. **行业打标**：根据观点内容，从以下申万一级行业中选取涉及的行业（可多选，无则为空数组）：
   {industries}
5. **概念打标**：根据观点内容，从以下概念/主题中选取涉及的概念（可多选，无则为空数组）：
   {concepts}
6. **情感判断**：判断该基金经理观点的整体情感倾向，只能从以下值中选择一个：
   {sentiments}
...
- original_view 应尽量保留原文表述，不要过度概括
